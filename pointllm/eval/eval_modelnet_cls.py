...
PROMPT_LISTS = [
    ...
    "Out of these three objects, which object is largest?",
    "How many objects are there? answer in number"
]
...
def load_dataset(config_path, split, subset_nums, use_color, data_path=None):
    if data_path:
        print(f"Loading {split} split of ModelNet datasets from {data_path}.")
        dataset = ModelNet(config_path=config_path, split=split, subset_nums=subset_nums, 
...
def main(args):
    ...
    # ② 出力ディレクトリをCLIで上書きできるように  
    if args.output_dir is None:
        args.output_dir = os.path.join(args.model_name, "evaluation")
        
    # ③ カスタムデータパスがある場合、出力ファイル名にノイズバリアント名を含める
    if args.data_path:
        # パスからノイズバリアント名を抽出
        variant_name = os.path.basename(os.path.dirname(args.data_path))
        args.output_file = f"ModelNet_classification_{variant_name}_prompt{args.prompt_index}.json"
    else:
    ...
    if not os.path.exists(args.output_file_path):
        ...
        dataset = load_dataset(config_path=args.config_path, 
                               split=args.split, 
                               subset_nums=args.subset_nums, 
                               use_color=args.use_color, 
                               data_path=args.data_path)  # data_pathを追加
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_path", type=str, default=None,
        help="YAML for ModelNet (point_dir, split_file, label_file)。省略時はデフォルト")
    parser.add_argument("--output_dir", type=str, default=None,
        help="結果 json を入れるディレクトリ。省略時は <model_name>/evaluation/")
    parser.add_argument("--data_path", type=str, default="/groups/gag51404/ide/PointLLM/data/modelnet40_data/modelnet40_test_8192pts_fps.dat",
        help="カスタムModelNetデータファイルへのパス。指定時はconfig_pathより優先")
    # parser.add_argument("--model_name", type=str, \
    #     default="RunsenXu/PointLLM_7B_v1.2")
    parser.add_argument("--model_name", type=str, \
        default="/groups/gag51404/ide/PointLLM/outputs/PointLLM_train_stage1/PointLLM_train_stage1_original")
