...
class DataCollatorForPointTextDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'point_clouds' in instances[0]:
            ...
            # Check if all elements are tensors (single point cloud) or lists (multiple point clouds)
            if all(x is not None for x in point_clouds):
                # If first element is a list, we have multiple point clouds
                if isinstance(point_clouds[0], list):
                    batch['point_clouds'] = point_clouds  # Keep as nested lists
                # If first element is a tensor, we have single point clouds
                    ...
                    batch['point_clouds'] = point_clouds  # Return as lists for different shapes
            else:
