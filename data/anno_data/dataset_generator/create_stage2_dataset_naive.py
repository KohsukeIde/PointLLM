...
class Stage2NaiveDatasetGenerator:
    """Stage2用のナイーブな複数点群データセット生成器（識別子なし版）"""
    ...
    def _create_stage2_naive_sample(self, source_samples: List[Dict[str, Any]], target_index: int) -> Dict[str, Any]:
        """Stage2用のナイーブなサンプルを作成（識別子なし）"""
        ...
        # ★ナイーブ版の主要変更点: 識別子を使わず、汎用プレースホルダー<point>を繰り返す
        ...
        new_sample = {
            ...
            "metadata": {
                ...
                "ablation_type": "naive_no_identifiers"
            }
        }
        
        return new_sample
    
    def generate_stage2_naive_dataset(self, num_samples: int = 70000) -> List[Dict[str, Any]]:
        """Stage2用のナイーブなデータセットを生成（識別子なし）"""
        if len(self.original_data) < 3:
            raise ValueError("Need at least 3 samples in original data to generate stage2 dataset")
        
        print(f"Generating {num_samples} Stage2 Naive samples (without identifiers)...")
        print(f"Source data size: {len(self.original_data)}")
        
        new_dataset = []
        
        for i in tqdm(range(num_samples), desc="Generating Stage2 Naive Data"):
            ...
            # 4. Stage2ナイーブサンプルを作成
            stage2_sample = self._create_stage2_naive_sample(source_samples, target_index)
            new_dataset.append(stage2_sample)
        
        print(f"Generated {len(new_dataset)} Stage2 Naive samples")
        return new_dataset
    
    def save_dataset(self, dataset: List[Dict[str, Any]], filename: str = "ablation_complex_stage2_naive_70K.json"):
        """データセットを保存"""
        os.makedirs(self.output_dir, exist_ok=True)
        
        output_path = os.path.join(self.output_dir, filename)
        
        print(f"Saving naive dataset to {output_path}...")
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump(dataset, f, indent=2, ensure_ascii=False)
        
        # 統計情報を生成・保存
        stats = self._generate_statistics(dataset)
        stats_path = os.path.join(self.output_dir, "ablation_complex_stage2_naive_70K_stats.json")
        with open(stats_path, 'w', encoding='utf-8') as f:
            json.dump(stats, f, indent=2, ensure_ascii=False)
        
        print(f"Naive dataset saved to {output_path}")
    ...
    def _generate_statistics(self, dataset: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "generation_info": {
                ...
                "ablation_type": "naive_no_identifiers",
                "identifier_format": "<point> <point> ... (repeated per num_point_clouds)"
            }
        }
...
def main():
    parser = argparse.ArgumentParser(description="Generate Stage2 naive multi-object point cloud dataset (without identifiers)")
    ...
    parser.add_argument("--num_samples", type=int, default=70000,
                        help="Number of Stage2 naive samples to generate")
    parser.add_argument("--seed", type=int, default=42,
                        help="Random seed for reproducibility")
    
    args = parser.parse_args()
    
    print("Stage2 Naive Multi-Object Point Cloud Dataset Generator (Ablation Study)")
    print("=" * 80)
    print(f"Data directory: {args.data_dir}")
    print(f"Output directory: {args.output_dir}")
    print(f"Number of samples: {args.num_samples}")
    print(f"Random seed: {args.seed}")
    print("Ablation type: No identifier tokens (uses generic <point> placeholders)")
    
    # データセット生成器を作成
    generator = Stage2NaiveDatasetGenerator(
    ...
    # Stage2ナイーブデータセット生成
    dataset = generator.generate_stage2_naive_dataset(num_samples=args.num_samples)
    
    # データセット保存
    output_path = generator.save_dataset(dataset)
    
    print("\nStage2 naive dataset generation completed!")
    print(f"Output: {output_path}")
    print("\nNext steps:")
    print("1. Use this dataset for ablation study training")
    print("2. Compare results with identifier-based approach")
    print("3. Evaluate the effectiveness of identifier tokens")
